...
Enody is a federated system where Runtimes communicate via message passing. An Environment is the entry point for device discovery and resource access — it owns connections to remote Runtimes and exposes the resources they contain. Discovery environments (UsbEnvironment, WifiEnvironment) actively scan for devices. Fixtures are addressed by identifier, independent of which Host owns them, enabling mesh routing and location transparency.
...
```
enody/
  __init__.py        # Re-exports core types, USB/WiFi environments, token helpers, device classes
  interface.py       # High-level UsbEnvironment, WifiEnvironment, Runtime, Host, Fixture, Source, Emitter, Token, TokenStore, WifiConnection, UpdateTarget classes
  ...
  cli.py             # CLI entry point (enody list, info, monitor, wifi-scan, wifi-join, wifi-setup, wifi-generate-token, download-spectral-data, update)
  data/
...
UsbEnvironment()                       # Not thread-safe
  ...
  .next_runtime_event() → EnvironmentRuntimeEvent

WifiEnvironment(tokens=None, timeout_ms=800, excluded_host_ids=None)
  # tokens=None loads TokenStore.load().tokens(); [] disables saved-token discovery
  .runtimes() → list[Runtime]
  .start_discovery(), .stop_discovery()
  .next_runtime_event() → EnvironmentRuntimeEvent
  .exclude_host_id(host_id), .remove_excluded_host_id(host_id)

EnvironmentRuntimeEvent
  .kind() → "arrived" | "left"
  .runtime() → Runtime

Token(host_id: str, key_id: str, data: list[int])
  .host_id(), .key_id(), .data()
  .to_dict() / Token.from_dict(data)
  repr redacts token data

TokenStore
  TokenStore.load(), TokenStore.load_from_path(path)
  TokenStore.save_token(token)
  .tokens() → list[Token]
  .upsert(token), .save(), .save_to_path(path)

WifiNetwork
  .ssid(), .bssid(), .channel(), .rssi(), .auth()

WifiDiscoveredDevice
  .endpoint(), .host_id(), .firmware_version(), .is_ep01()
  .is_token_generation_candidate()

WifiConnection
  .discover_token_generation_devices(timeout_ms=800) → list[WifiDiscoveredDevice]
  .generate_token_from_discovered_device(device, on_approval=None) → Token
  .generate_token_from_endpoint(endpoint, on_approval=None) → Token
  .runtime_from_discovered_device(token, device) → Runtime
  .runtime_from_endpoint(token, endpoint) → Runtime
...
Primary Home Assistant style flow:
  token = enody.generate_wifi_token(on_approval=callback, save=False)

Behavior:
  1. Discover token-generation candidates over mDNS using WifiConnection.discover_token_generation_devices().
  2. If exactly one device is found, run unauthenticated WiFi pairing over Noise_NN.
  3. Device sends approval instructions via callback; user physically approves on EP01.
  4. Returned Token is verified by reconnecting with the authenticated WiFi Noise PSK flow and reading host.identifier().
  5. Verification retries 8 times with 500ms delay by default.
  6. Token can be stored by Home Assistant via token.to_dict(), or by enody via TokenStore.save_token(token).

Helpers:
  generate_wifi_token(device=None, endpoint=None, timeout_ms=800, on_approval=None, verify=True, save=False)
  verify_wifi_token_from_discovered_device(token, device)
  verify_wifi_token_from_endpoint(token, endpoint)
  discover_runtimes(tokens=None, include_usb=True, include_wifi=True, wifi_timeout_ms=800)

Saved-token runtime discovery:
...
## CLI (enody command)
  ...
  enody info                          # Show detailed USB/WiFi device/fixture/source/emitter info
  enody monitor                       # Stream device log output
  enody wifi-scan                     # Scan WiFi networks through first USB EP01
  enody wifi-join SSID                # Join a WiFi network through first USB EP01
  enody wifi-setup                    # Interactive scan/join and USB-authenticated token save
  enody wifi-generate-token           # mDNS WiFi pairing, verification, and token save
